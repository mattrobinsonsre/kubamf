...
// Analyze container resource provisioning and return worst case
const getResourceProvisioningStatus = (pod, metrics, type) => {
  if (!pod.spec?.containers || !metrics) {
    return { status: 'unknown', tooltip: '', color: 'text-gray-500' }
  }

  const worstCase = {
    severity: 0, // 0=good, 1=under limit, 2=under request, 3=over request, 4=over limit
    container: null,
    message: '',
    percentage: 0
  }

  // We need to get per-container metrics
  // The metrics object contains total pod metrics, but we need container-level analysis
  // For now, we'll use total pod metrics and compare against total container resources

  let totalRequests = 0
  let totalLimits = 0
  let hasLimits = false
  let hasRequests = false
  const containersWithoutRequests = []
  const containersWithoutLimits = []
  const containersWithoutAny = []

  pod.spec.containers.forEach(container => {
    let containerHasRequest = false
    let containerHasLimit = false

    if (container.resources) {
      if (type === 'cpu') {
        if (container.resources.requests?.cpu) {
          totalRequests += parseResourceValue(container.resources.requests.cpu, 'cpu')
          hasRequests = true
          containerHasRequest = true
        }
        if (container.resources.limits?.cpu) {
          totalLimits += parseResourceValue(container.resources.limits.cpu, 'cpu')
          hasLimits = true
          containerHasLimit = true
        }
      } else if (type === 'memory') {
        if (container.resources.requests?.memory) {
          totalRequests += parseResourceValue(container.resources.requests.memory, 'memory')
          hasRequests = true
          containerHasRequest = true
        }
        if (container.resources.limits?.memory) {
          totalLimits += parseResourceValue(container.resources.limits.memory, 'memory')
          hasLimits = true
          containerHasLimit = true
        }
      }
    ...
    // Track which containers are missing specifications
    if (!containerHasRequest && !containerHasLimit) {
      containersWithoutAny.push(container.name)
    } else if (!containerHasRequest) {
      containersWithoutRequests.push(container.name)
    } else if (!containerHasLimit) {
      containersWithoutLimits.push(container.name)
    }
  })

  // Get actual usage
  const usage = parseResourceValue(metrics[type], type)

  // Check for containers without any specifications
  if (containersWithoutAny.length > 0) {
    const containerList = containersWithoutAny.length === pod.spec.containers.length
      ? 'All containers'
      : `Containers: ${containersWithoutAny.join(', ')}`
    return {
      status: 'unspecified',
      tooltip: `No ${type.toUpperCase()} requests or limits (${containerList})`,
      color: 'text-yellow-500'
    }
  }

  // Check if some containers are missing requests
  if (containersWithoutRequests.length > 0) {
    return {
      status: 'missing-requests',
      tooltip: `Missing ${type.toUpperCase()} requests (${containersWithoutRequests.join(', ')})`,
      color: 'text-yellow-500'
    }
  }

  // Check if some containers are missing limits
  if (containersWithoutLimits.length > 0) {
    return {
      status: 'missing-limits',
      tooltip: `Missing ${type.toUpperCase()} limits (${containersWithoutLimits.join(', ')})`,
      color: 'text-yellow-500'
    }
  }

  // Check against limits (worst case - over 80% of limit)
  if (hasLimits && totalLimits > 0) {
    const limitPercent = (usage / totalLimits) * 100
    if (limitPercent > 80) {
      return {
        status: 'critical',
        tooltip: `Over 80% of limit (${limitPercent.toFixed(0)}%)`,
        color: 'text-red-600'
      }
    }
  }

  // Check against requests (over 95% of request)
  if (hasRequests && totalRequests > 0) {
    const requestPercent = (usage / totalRequests) * 100
    if (requestPercent > 95) {
      return {
        status: 'warning',
        tooltip: `Over 95% of request (${requestPercent.toFixed(0)}%)`,
        color: 'text-orange-500'
      }
    }
    // Under 50% of request (over-provisioned)
    if (requestPercent < 50) {
      return {
        status: 'over-provisioned',
        tooltip: `Under 50% of request (${requestPercent.toFixed(0)}%)`,
        color: 'text-blue-500'
      }
    }
  }

  // Check if under 50% of limit (if no request)
  if (!hasRequests && hasLimits && totalLimits > 0) {
    const limitPercent = (usage / totalLimits) * 100
    if (limitPercent < 50) {
      return {
        status: 'over-provisioned',
        tooltip: `Under 50% of limit (${limitPercent.toFixed(0)}%)`,
        color: 'text-blue-500'
      }
    }
  }

  // All good - properly provisioned
  return {
    status: 'good',
    tooltip: 'Properly provisioned',
